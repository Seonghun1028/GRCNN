...
def plot_results(
    ...
    :return:
    """
    gs , grasp_point = detect_grasps(grasp_q_img, grasp_angle_img, width_img=grasp_width_img, no_grasps=no_grasps)  ######### grasp point 추가 #########
    ...
    for g in gs:
        g.plot(ax)
    ax.plot(grasp_point[1],grasp_point[0],'o-g', linewidth=3, markersize=3)                  ############## 추가 ##############
    ax.set_title('Grasp')
    ax.legend(['score : ' + str(score)], loc='upper right', fontsize='xx-small')             ############## 추가 ##############
    ax.axis('off')
...
def save_results(rgb_img, grasp_q_img, grasp_angle_img, score, depth_img=None, no_grasps=1, grasp_width_img=None):          ######### score 추가 #######
    """
    Plot the output of a network
    :param rgb_img: RGB Image
    :param depth_img: Depth Image
    ...
    :return:
    """
    gs , grasp_point = detect_grasps(grasp_q_img, grasp_angle_img, width_img=grasp_width_img, no_grasps=no_grasps)      ####### grasp_point 추가 #######
