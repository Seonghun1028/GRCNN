...
class Calibration:
    def __init__(self,
        ...
        self.observed_pts = []
        self.observed_pix = []
        self.camera2world = np.eye(4) # 4x4 대각행렬(주대각선 성분 = 1)

        homedir = os.path.join(os.path.expanduser('~'), "robotic-grasping/grasp-comms")
        self.move_completed = os.path.join(homedir, "move_completed.npy")
        self.tool_position = os.path.join(homedir, "tool_position.npy")
        # homedir = os.path.join(os.path.expanduser('~'), "robotic-grasping/grasp-comms")
        # self.move_completed = os.path.join(homedir, "move_completed.npy")
        # np.save(self.move_completed,np.array([]))
        # self.tool_position = os.path.join(homedir, "tool_position.npy")
        # np.save(self.tool_position,np.array([]))
    ...
    def _generate_grid(self):
        ...
        gridspace_x = np.linspace(self.workspace_limits[0][0], self.workspace_limits[0][1],
                                  1 + (self.workspace_limits[0][1] - self.workspace_limits[0][
                                      0]) / self.calib_grid_step)                                     # array([0.55, 0.6, 0.65])
        gridspace_y = np.linspace(self.workspace_limits[1][0], self.workspace_limits[1][1],
                                  1 + (self.workspace_limits[1][1] - self.workspace_limits[1][
                                      0]) / self.calib_grid_step)                                     # array([-0.1, -0.15, -0.2])
        gridspace_z = np.linspace(self.workspace_limits[2][0], self.workspace_limits[2][1],
                                  1 + (self.workspace_limits[2][1] - self.workspace_limits[2][
                                      0]) / self.calib_grid_step)                                     # array([0.0, 0.05, 0.1, 0.15, 0.2])
        calib_grid_x, calib_grid_y, calib_grid_z = np.meshgrid(gridspace_x, gridspace_y, gridspace_z) # 격자 생성(각각의 shape는 (3,3,5))
        num_calib_grid_pts = calib_grid_x.shape[0] * calib_grid_x.shape[1] * calib_grid_x.shape[2]    # 격자점의 총 갯수 (3*3*5)
        calib_grid_x.shape = (num_calib_grid_pts, 1) # (3*3*5, 1) 즉 (45,1)의 크기로 flatten
        calib_grid_y.shape = (num_calib_grid_pts, 1)
        calib_grid_z.shape = (num_calib_grid_pts, 1)
        calib_grid_pts = np.concatenate((calib_grid_x, calib_grid_y, calib_grid_z), axis=1) # (45,3)의 크기로 배열 합치기
    ...
    def run(self):
        ...
        calib_grid_pts = self._generate_grid() #_generate_grid() 함수로 부터 calib_grid pts를 return받음

        logging.info('Total grid points: ', calib_grid_pts.shape[0]) # 총 격자 점 갯수는 45개
        ...
        for tool_position in calib_grid_pts:
            ...
            # termination criteria(TERM_CRITERIA_EP : 정해둔 정확도에 다다르면 알고리즘 반복을 멈춤(0.001),
            # Term_CRITERIA_MAX_ITER : 지정한 반복 수만큼 지나면 알고리즘 멈춤(30), 두 코드를 더함으로써 두 조건 중 하나라도 만족하면 알고리즘 멈춤)
            image_bundle = self.camera.get_image_bundle() # camera.py의 get_image_bundle() 함수 호출
            camera_color_img = image_bundle['rgb'] # 'rgb'에 해당되는 (480,640,3)의 이미지 배열을 변수로 저장
            camera_depth_img = image_bundle['aligned_depth'] # 'aligned_depth'에 해당되는 (480,640,1)의 이미지 배열을 변수로 저장
            bgr_color_data = cv2.cvtColor(camera_color_img, cv2.COLOR_RGB2BGR)
            gray_data = cv2.cvtColor(bgr_color_data, cv2.COLOR_RGB2GRAY)  #RGB -> BGR????
            checkerboard_found, corners = cv2.findChessboardCorners(gray_data, checkerboard_size, None,
                                                                    cv2.CALIB_CB_ADAPTIVE_THRESH) # 패턴이 감지되었는지 여부에 따라 ret 출력은 boolean, corners 출력은 감지된 코너의 배열
            if checkerboard_found:
                corners_refined = cv2.cornerSubPix(gray_data, corners, (3, 3), (-1, -1), refine_criteria)
                # 코너 위치 개선 함수
				# cv2.cornerSubPix(image, corners, winSize(search window size), zeroZone, criteria)
